...
if (process.env.SMTP_ENABLE && parseInt(process.env.SMTP_ENABLE) == 1) {
  try {
    ...
    transporters[Transport.SMTP] = nodemailer.createTransport(smtpConfig);
...
export const sendEmail = async (
  ...
  try {
    ...
    const result = await transporters[transport].sendMail(mailOptions);
 ...
 */
export const applicationAcceptedEmailTemplate = (
 ...
 */
export const applicationShortlistedEmailTemplate = (
  candidateName: string,
  jobTitle: string,
  companyName: string,
  employerName: string
