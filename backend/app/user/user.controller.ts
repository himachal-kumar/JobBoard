...
export const refreshUserToken = asyncHandler(async (req: Request, res: Response) => {
  ...
  const newTokens = await userService.refreshUserToken(refreshToken);
  
  if (!newTokens) {
    throw createHttpError(401, "Invalid or expired refresh token");
  }

  res.send(createResponse(newTokens, "Token refreshed successfully"));
});

export const logoutUser = asyncHandler(async (req: AuthRequest, res: Response) => {
  const success = await userService.logoutUser(req.user!._id);
  
  if (!success) {
    throw createHttpError(500, "Failed to logout");
  }

  res.send(createResponse(null, "Logout successful"));
});
...
export const createAdminUser = asyncHandler(async (req: Request, res: Response) => {
  ...
  res.status(201).send(createResponse(result, "Admin user created successfully"));
});
...
export const updateUserByAdmin = asyncHandler(async (req: Request, res: Response) => {
  ...
  const { password, refreshToken, provider, ...safeUpdateData } = updateData;
  
  const result = await userService.editUser(id, safeUpdateData);
